...
-(void)getDataInfo:(QHInvokedUrlCommand *)command {
    NSDictionary *dic = [QHLoanDoor share].dataInfo;
    if (dic) {
        QHPluginResult *result = [QHPluginResult resultWithStatus:QHCommandStatus_OK messageAsDictionary:dic];
        [self.commandDelegate sendPluginResult:result callbackId:command.callbackId];

    }else {
         QHPluginResult *result = [QHPluginResult resultWithStatus:QHCommandStatus_ERROR messageAsDictionary:QHMsgLoseParas];
        [self.commandDelegate sendPluginResult:result callbackId:command.callbackId];
    }
...
-(void)executeH5InfoAction:(QHInvokedUrlCommand *)command {
    [[QHLoanDoor share] getCommandDelegate:self.commandDelegate InvokedUrlCommand:command];
    id params = [command argumentAtIndex:0 withDefault:[NSNull null]];
    
    QHViewController *vc = (QHViewController *)self.viewController;
    if (vc.basicDelegate && [vc.basicDelegate respondsToSelector:@selector(executeH5InfoAction:)]) {
       NSDictionary *dic =  [vc.basicDelegate executeH5InfoAction:params];
        if (!dic) {
            dic = @{};
        }
//        QHPluginResult *result = [QHPluginResult resultWithStatus:QHCommandStatus_OK messageAsDictionary:dic];
