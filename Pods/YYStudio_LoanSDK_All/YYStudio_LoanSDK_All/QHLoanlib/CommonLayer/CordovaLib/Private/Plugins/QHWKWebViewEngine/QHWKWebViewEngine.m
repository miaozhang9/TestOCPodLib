...
#define QH_BRIDGE_NAME @"cordova"
#define QH_WKWEBVIEW_FILE_URL_LOAD_SELECTOR @"loadFileURL:allowingReadAccessToURL:"

@interface QHWKWeakScriptMessageHandler : NSObject <WKScriptMessageHandler>
...
@implementation QHWKWebViewEngine
...
- (WKWebViewConfiguration*) createConfigurationFromSettings:(NSDictionary*)settings
{
    ...
    configuration.processPool = [[QHWKProcessPoolFactory sharedFactory] sharedProcessPool];
    if (settings == nil) {
        return configuration;
    }

    configuration.allowsInlineMediaPlayback = [settings qh_cordovaBoolSettingForKey:@"AllowInlineMediaPlayback" defaultValue:NO];
    configuration.mediaPlaybackRequiresUserAction = [settings qh_cordovaBoolSettingForKey:@"MediaPlaybackRequiresUserAction" defaultValue:YES];
    configuration.suppressesIncrementalRendering = [settings qh_cordovaBoolSettingForKey:@"SuppressesIncrementalRendering" defaultValue:NO];
    configuration.mediaPlaybackAllowsAirPlay = [settings qh_cordovaBoolSettingForKey:@"MediaPlaybackAllowsAirPlay" defaultValue:YES];
    return configuration;
}
...
- (void)pluginInitialize
{
    ...
    self.uiDelegate = [[QHWKWebViewUIDelegate alloc] initWithTitle:[[NSBundle bundleForClass:[self class]] objectForInfoDictionaryKey:@"CFBundleDisplayName"]];

    QHWKWeakScriptMessageHandler *weakScriptMessageHandler = [[QHWKWeakScriptMessageHandler alloc] initWithScriptMessageHandler:self];

    WKUserContentController* userContentController = [[WKUserContentController alloc] init];
    [userContentController addScriptMessageHandler:weakScriptMessageHandler name:QH_BRIDGE_NAME];
    ...
    if (IsAtLeastiOSVersion(@"9.0") && [self.viewController isKindOfClass:[QHViewController class]]) {
        wkWebView.customUserAgent = ((QHViewController*) self.viewController).userAgent;
    }
    ...
    if ([self.viewController conformsToProtocol:@protocol(WKScriptMessageHandler)]) {
        [wkWebView.configuration.userContentController addScriptMessageHandler:(id < WKScriptMessageHandler >)self.viewController name:QH_BRIDGE_NAME];
    }

    [self updateSettings:settings];

    // check if content thread has died on resume
    NSLog(@"%@", @"QHWKWebViewEngine will reload WKWebView if required on resume");
...
- (void) onAppWillEnterForeground:(NSNotification*)notification {
    if ([self shouldReloadWebView]) {
        NSLog(@"%@", @"QHWKWebViewEngine reloading!");
        [(WKWebView*)_engineWebView reload];
    }
}
...
#ifdef DEBUG
    NSLog(@"%@", @"QHWKWebViewEngine shouldReloadWebView::");
    NSLog(@"QHWKWebViewEngine shouldReloadWebView title: %@", title);
    NSLog(@"QHWKWebViewEngine shouldReloadWebView location: %@", [location absoluteString]);
    NSLog(@"QHWKWebViewEngine shouldReloadWebView reload: %u", reload);
#endif
...
- (id)loadRequest:(NSURLRequest*)request
{
    if ([self canLoadRequest:request]) { // can load, differentiate between file urls and other schemes
        if (request.URL.fileURL) {
            SEL wk_sel = NSSelectorFromString(QH_WKWEBVIEW_FILE_URL_LOAD_SELECTOR);
...
- (void)updateSettings:(NSDictionary*)settings
{
    ...
    wkWebView.configuration.preferences.minimumFontSize = [settings qh_cordovaFloatSettingForKey:@"MinimumFontSize" defaultValue:0.0];
    ...
    NSString* decelerationSetting = [settings qh_cordovaSettingForKey:@"WKWebViewDecelerationSpeed"];
    if (!decelerationSetting) {
        // Fallback to the UIWebView-named preference
        decelerationSetting = [settings qh_cordovaSettingForKey:@"UIWebViewDecelerationSpeed"];
    }
    ...
    wkWebView.allowsBackForwardNavigationGestures = [settings qh_cordovaBoolSettingForKey:@"AllowBackForwardNavigationGestures" defaultValue:NO];
}

- (void)updateWithInfo:(NSDictionary*)info
{
    NSDictionary* scriptMessageHandlers = [info objectForKey:kQHWebViewEngineScriptMessageHandlers];
    NSDictionary* settings = [info objectForKey:kQHWebViewEngineWebViewPreferences];
    id navigationDelegate = [info objectForKey:kQHWebViewEngineWKNavigationDelegate];
    id uiDelegate = [info objectForKey:kQHWebViewEngineWKUIDelegate];
...
{
    if (![message.name isEqualToString:QH_BRIDGE_NAME]) {
        return;
    }
...
{
    [[NSNotificationCenter defaultCenter] postNotification:[NSNotification notificationWithName:QHPluginResetNotification object:webView]];
}
...
- (void)webView:(WKWebView*)webView didFinishNavigation:(WKNavigation*)navigation
{
    ...
    [[NSNotificationCenter defaultCenter] postNotification:[NSNotification notificationWithName:QHPageDidLoadNotification object:webView]];
}
...
- (void)webView:(WKWebView*)theWebView didFailNavigation:(WKNavigation*)navigation withError:(NSError*)error
{
    ...
    [QHUserAgentUtil releaseLock:vc.userAgentLockToken];
     ...
     [[NSNotificationCenter defaultCenter] postNotification:[NSNotification notificationWithName:QHPageDidFailNotification object:theWebView]];
...
- (void)allowsBackForwardNavigationGestures:(QHInvokedUrlCommand*)command;
{
...
#pragma mark - QHWKWeakScriptMessageHandler

@implementation QHWKWeakScriptMessageHandler
